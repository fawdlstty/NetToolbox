...
public:
	page_SysInfo (NetToolboxWnd *parent): page_base (parent) {
		m_sysinfo_trademark->SetText (tool_Encoding::get_T (tool_SysInfo::get_trademark_name ()));
		m_sysinfo_version->SetText (tool_Encoding::get_T (tool_SysInfo::get_system_info ()));
		auto[cpu1, cpu2] = tool_SysInfo::get_cpu_info ();
		m_sysinfo_cpu1->SetText (tool_Encoding::get_T (cpu1));
		m_sysinfo_cpu2->SetText (tool_Encoding::get_T (cpu2));
		m_sysinfo_mem->SetText (tool_SysInfo::get_memory_info ());
		//
		std::thread ([this] () {
			m_sysinfo_bios_id->SetText (tool_Encoding::get_T (tool_WMI::get_bios_id ()));
			m_sysinfo_cpu_id->SetText (tool_Encoding::get_T (tool_WMI::get_cpu_id ()));
			m_sysinfo_disk_id->SetText (tool_Encoding::get_T (tool_WMI::get_disk_id ()));
			m_sysinfo_mainboard_id->SetText (tool_Encoding::get_T (tool_WMI::get_mainboard_id ()));
			m_sysinfo_netcard_mac->SetText (tool_Encoding::get_T (tool_WMI::get_netcard_mac ()));
			m_sysinfo_netcard_id->SetText (tool_Encoding::get_T (tool_WMI::get_netcard_id ()));
		}).detach ();
	}
